...
function obtenerPagoPorId(id) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        const select_AlumnoId = $('#alumno_update');
        ...
        itemID.val(id)
        codigoId.val(codigo);
         select_AlumnoId.val(Alumno);
        select_pagoId.val(Metodo_Pago);
        monto.val(Monto);
        descuentosId.val(Descuento);
        observacionId.val(Observacion);
        totalId.val(Total);
        selectionDinamic(id_Alumno,Alumno,Metodo_Pago);
      // Limpiar opciones existentes en el select_status
      $('#select_status').empty();

      // Agregar opciones predeterminadas
      $('#select_status').append('<option value="default">Seleccione el estado del pago</option>');
      $('#select_status').append('<option value="1">Pagado</option>');
      $('#select_status').append('<option value="0">Pendiente</option>');
...
function selectionDinamic (id_Alumno,Alumno,Metodo_Pago) {
    ...
    $.ajax({
        ...
        console.log(responseJson);
            ...
            // Crear un conjunto para asegurarse de que no se repitan los estados
            const uniqueStatuses = new Set();

        

            if(Alumno){
                const selectedAlumno=   responseJson.data.find(item => item.id === id_Alumno)
                console.log(selectedAlumno);
                
                  // Verificar si se encontró el tutor
                if (selectedAlumno) {
                    alumnoSelect.append(`<option value="${id_Alumno}" selected  >${selectedAlumno.Alumno}</option>`);
                }
            }

            if(Metodo_Pago){
                const selectedTutor=   responseJson.data.find(item => item.Metodo_Pago == Metodo_Pago)
                
                  // Verificar si se encontró el tutor
                if (selectedTutor) {
                    metodoSelect.append(`<option value="${selectedTutor.Metodo_Pago}" selected  >${selectedTutor.Metodo_Pago}</option>`);
                }
            }
